...
import { riders, riderFinances, clientBalances } from "@/data/mock-data";
import {
...
const allDailyData = Array.from({ length: 90 }, (_, i) => {
  const dateObj = new Date(2026, 2, 7);
...
const riderPerformance = riderFinances.map((r) => {
  ...
  const avgRating = Math.min(5, +(4.1 + Math.random() * 0.8).toFixed(1));
  const deliveriesPerHour = +(r.totalDeliveries / 8).toFixed(1);
  const revenuePerHour = +(r.totalCollected / 8).toFixed(0);
  const efficiency = Math.round((sla * 0.4) + (avgRating / 5 * 100 * 0.3) + ((1 - cancelRate / 10) * 100 * 0.3));
  return { ...r, avgDeliveryTime, sla, cancelRate, avgRating, deliveriesPerHour, revenuePerHour: +revenuePerHour, efficiency };
...
export default function Metrics() {
  const [dateFrom, setDateFrom] = useState<Date | undefined>(() => {
    const d = new Date(2026, 2, 7); d.setDate(d.getDate() - 29); return d;
  });
  const [dateTo, setDateTo] = useState<Date | undefined>(new Date(2026, 2, 7));
  ...
  const dailyData = useMemo(() => {
    return allDailyData.filter(d => {
      ...
      if (dateTo) { const end = new Date(dateTo); end.setHours(23, 59, 59); if (d.fullDate > end) return false; }
      return true;
    });
  }, [dateFrom, dateTo]);
  ...
  const calcTrend = (key: "completed" | "revenue" | "avgTime") =>
    olderHalf.length > 0 ? ((mean(recentHalf.map(d => d[key])) - mean(olderHalf.map(d => d[key]))) / mean(olderHalf.map(d => d[key]))) * 100 : 0;

  const ordersWoW = calcTrend("completed");
  const revenueWoW = calcTrend("revenue");
  const timeWoW = calcTrend("avgTime");
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {PRESETS.map((p) => (
            <Button
              ...
              variant="outline"
              size="sm"
              className={cn("text-xs h-7 px-2", daysInRange === p.days && "bg-primary text-primary-foreground hover:bg-primary/90")}
      ...
      <Tabs defaultValue="operational">
        ...
        <TabsContent value="operational">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent className="p-4">
                ...
                <div className="mt-2 grid grid-cols-3 gap-2 text-[10px]">
                  <div className="bg-muted/50 rounded p-2 text-center"><p className="text-muted-foreground">Hora pico</p><p className="font-bold text-foreground">13:00-14:00</p><p className="text-muted-foreground">52 pedidos</p></div>
                  <div className="bg-muted/50 rounded p-2 text-center"><p className="text-muted-foreground">Hora valle</p><p className="font-bold text-foreground">7:00-8:00</p><p className="text-muted-foreground">12 pedidos</p></div>
                  <div className="bg-muted/50 rounded p-2 text-center"><p className="text-muted-foreground">Ratio pico/valle</p><p className="font-bold text-foreground">4.3x</p><p className="text-muted-foreground">Concentración alta</p></div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="py-3 px-4 border-b">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="text-[10px]">{daysInRange}d</Badge>
                </div>
              </CardHeader>
              <CardContent className="p-4">
                <div className="h-[240px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={last7}>
                      ...
                      <YAxis yAxisId="left" tick={{ fontSize: 10 }} domain={[15, 40]} />
                      ...
                      <Line yAxisId="right" type="monotone" dataKey="slaCompliance" stroke="hsl(160, 60%, 42%)" strokeWidth={2} name="SLA %" dot={{ r: 3 }} />
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
                <div className="mt-2 grid grid-cols-3 gap-2 text-[10px]">
                  <div className="bg-muted/50 rounded p-2 text-center"><p className="text-muted-foreground">SLA promedio</p><p className="font-bold text-foreground">{mean(last7.map(d => d.slaCompliance)).toFixed(1)}%</p></div>
                  <div className="bg-muted/50 rounded p-2 text-center"><p className="text-muted-foreground">Mejor día</p><p className="font-bold text-foreground">{last7.length ? Math.max(...last7.map(d => d.slaCompliance)) : 0}%</p></div>
                  <div className="bg-muted/50 rounded p-2 text-center"><p className="text-muted-foreground">Peor día</p><p className="font-bold text-foreground">{last7.length ? Math.min(...last7.map(d => d.slaCompliance)) : 0}%</p></div>
                </div>
              </CardContent>
            </Card>

            <Card>
              <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">Tasa de Éxito vs Incidencias</CardTitle></CardHeader>
              <CardContent className="p-4">
                <div className="h-[200px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={[
                          { name: "Completados", value: stats.totalOrders },
                          { name: "Cancelados", value: dailyData.reduce((s, d) => s + d.cancelled, 0) },
                          { name: "Devueltos", value: dailyData.reduce((s, d) => s + d.returned, 0) },
                        ]}
                ...
                {(() => {
                  const totalAll = dailyData.reduce((s, d) => s + d.total, 0) || 1;
                  const canc = dailyData.reduce((s, d) => s + d.cancelled, 0);
                  const ret = dailyData.reduce((s, d) => s + d.returned, 0);
                  return (
                    <div className="grid grid-cols-3 gap-2 text-[10px] mt-2">
                      <div className="bg-success/10 rounded p-2 text-center"><p className="font-bold text-foreground">{((stats.totalOrders / totalAll) * 100).toFixed(1)}%</p><p className="text-muted-foreground">Tasa de éxito</p></div>
                      <div className="bg-destructive/10 rounded p-2 text-center"><p className="font-bold text-foreground">{((canc / totalAll) * 100).toFixed(1)}%</p><p className="text-muted-foreground">Tasa cancelación</p></div>
                      <div className="bg-warning/10 rounded p-2 text-center"><p className="font-bold text-foreground">{((ret / totalAll) * 100).toFixed(1)}%</p><p className="text-muted-foreground">Tasa devolución</p></div>
                    </div>
                  );
                })()}
              </CardContent>
            ...
            <Card>
              <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">Costo Unitario por Entrega</CardTitle></CardHeader>
              <CardContent className="p-4">
                <div className="h-[200px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={dailyData.slice(-14)}>
                      ...
                      <Area type="monotone" dataKey="costPerDelivery" stroke="hsl(var(--primary))" fill="hsl(220, 80%, 50%, 0.15)" strokeWidth={2} name="Costo" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
                <div className="grid grid-cols-2 gap-2 text-[10px] mt-2">
                  <div className="bg-muted/50 rounded p-2 text-center"><p className="text-muted-foreground">Promedio rango</p><p className="font-bold">{fmtDec(mean(dailyData.map(d => d.costPerDelivery)))}</p></div>
                  <div className="bg-muted/50 rounded p-2 text-center"><p className="text-muted-foreground">Meta</p><p className="font-bold text-success">$8.50</p></div>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <TabsContent value="trends">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card className="md:col-span-2">
              <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">Evolución Semanal — Febrero 2026</CardTitle></CardHeader>
              <CardContent className="p-4">
                <div className="h-[260px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={weeklyData}>
                      ...
                      <Line yAxisId="right" type="monotone" dataKey="sla" stroke="hsl(160, 60%, 42%)" strokeWidth={2} name="SLA %" dot={{ r: 4 }} />
                      <Line yAxisId="left" type="monotone" dataKey="avgTime" stroke="hsl(38, 92%, 50%)" strokeWidth={2} name="Tiempo prom (min)" dot={{ r: 4 }} />
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
            ...
            <Card className="md:col-span-2">
              <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">Comparativo Semanal Detallado</CardTitle></CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>Semana</TableHead><TableHead className="text-center">Pedidos</TableHead><TableHead className="text-right">Revenue</TableHead>
                      <TableHead className="text-center">Tiempo prom</TableHead><TableHead className="text-center">SLA</TableHead><TableHead className="text-right">Costo/entrega</TableHead>
                      <TableHead className="text-center">Riders</TableHead><TableHead className="text-right">Revenue/rider</TableHead><TableHead>Δ Pedidos</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {weeklyData.map((w, i) => {
                      const prev = i > 0 ? weeklyData[i - 1].orders : w.orders;
                      const growth = ((w.orders - prev) / prev) * 100;
                      return (
                        <TableRow key={w.week}>
                          ...
                          <TableCell className="text-center"><Badge className={`text-[10px] ${w.sla >= 93 ? "bg-success/15 text-success" : w.sla >= 90 ? "bg-warning/15 text-warning" : "bg-destructive/15 text-destructive"} border-0`}>{w.sla}%</Badge></TableCell>
            ...
            <Card className="md:col-span-2">
              <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">Tendencia de Ingresos Diarios ({daysInRange}d)</CardTitle></CardHeader>
              <CardContent className="p-4">
                <div className="h-[220px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={dailyData}>
                      <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                      <XAxis dataKey="date" tick={{ fontSize: 9 }} interval={Math.max(1, Math.floor(daysInRange / 15))} />
                      <YAxis tick={{ fontSize: 10 }} />
                      <Tooltip formatter={(v: number) => [fmt(v), "Revenue"]} />
                      <Area type="monotone" dataKey="revenue" stroke="hsl(var(--primary))" fill="hsl(220, 80%, 50%, 0.12)" strokeWidth={2} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
        ...
        <TabsContent value="strategic">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="h-[280px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={monthlyProjection}>
                      ...
                      <Tooltip />
                      <Bar yAxisId="left" dataKey="orders" fill="hsl(var(--primary))" radius={[3, 3, 0, 0]} name="Pedidos" />
                      <Line yAxisId="right" type="monotone" dataKey="revenue" stroke="hsl(160, 60%, 42%)" strokeWidth={2} name="Revenue" dot={{ r: 4 }} />
                      <Line yAxisId="left" type="monotone" dataKey="clients" stroke="hsl(38, 92%, 50%)" strokeWidth={2} name="Clientes" dot={{ r: 4 }} />
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
            ...
            <Card>
              <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">📐 Unit Economics</CardTitle></CardHeader>
              <CardContent className="p-4 space-y-3">
                <div className="space-y-2 text-sm">
                  {[
                    { label: "Ingreso promedio por pedido", value: fmtDec(stats.totalOrders ? stats.totalRevenue / stats.totalOrders : 0), trend: 3.2 },
                    { label: "Costo operativo por pedido", value: fmtDec(mean(dailyData.map(d => d.costPerDelivery))), trend: -5.1 },
                    { label: "Margen bruto por pedido", value: fmtDec(stats.totalOrders ? (stats.totalRevenue / stats.totalOrders) - mean(dailyData.map(d => d.costPerDelivery)) : 0), trend: 8.4 },
                    { label: "Pedidos por rider por día", value: (stats.avgOrders / totalRiders).toFixed(1), trend: 2.1 },
                    { label: "Revenue por rider mensual", value: fmt(Math.round(stats.totalRevenue / totalRiders)), trend: 6.3 },
            ...
            <Card>
              <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">🎯 Metas Estratégicas Q1 2026</CardTitle></CardHeader>
              <CardContent className="p-4 space-y-3">
                {[
                  ...
                  { label: "Cancelación < 3%", current: dailyData.reduce((s, d) => s + d.total, 0) ? (dailyData.reduce((s, d) => s + d.cancelled, 0) / dailyData.reduce((s, d) => s + d.total, 0)) * 100 : 0, target: 3, unit: "%", invert: true },
                ].map((goal) => {
                  const progress = goal.invert ? Math.min(100, (goal.target / (goal.current || 1)) * 100) : Math.min(100, (goal.current / goal.target) * 100);
                  return (
                    <div key={goal.label} className="space-y-1">
                      <div className="flex justify-between text-xs"><span className="text-muted-foreground">{goal.label}</span><span className="font-medium">{goal.current.toFixed(1)}{goal.unit} / {goal.target}{goal.unit}</span></div>
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${progress >= 90 ? "bg-success" : progress >= 70 ? "bg-warning" : "bg-destructive"}`} style={{ width: `${progress}%` }} />
                      </div>
                    </div>
                  );
                })}
            ...
            <Card className="md:col-span-2">
              <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">Proyección Mensual Detallada</CardTitle></CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>Mes</TableHead><TableHead className="text-center">Pedidos</TableHead><TableHead className="text-right">Revenue</TableHead>
                      <TableHead className="text-center">Clientes</TableHead><TableHead className="text-center">Riders</TableHead><TableHead className="text-right">Costo/pedido</TableHead>
                      <TableHead className="text-right">Margen est.</TableHead><TableHead>Crecimiento</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {monthlyProjection.map((m, i) => {
                      ...
                      return (
                        <TableRow key={m.month} className={isProjected ? "bg-muted/20" : ""}>
                          <TableCell className="text-sm font-medium">{m.month} {isProjected && <Badge variant="outline" className="text-[8px] ml-1">Est.</Badge>}</TableCell>
        ...
        <TabsContent value="riders">
          <Card>
            <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">Ranking de Rendimiento por Repartidor — Hoy</CardTitle></CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-10">#</TableHead><TableHead>Repartidor</TableHead><TableHead className="text-center">Entregas</TableHead>
                    <TableHead className="text-center">Entregas/hr</TableHead><TableHead className="text-center">Tiempo prom</TableHead><TableHead className="text-center">SLA</TableHead>
                    <TableHead className="text-center">Cancelación</TableHead><TableHead className="text-center">Rating</TableHead><TableHead className="text-right">Revenue/hr</TableHead>
                ...
                <TableBody>
                  {riderPerformance.map((r, i) => (
                    <TableRow key={r.riderId}>
                      <TableCell className="text-sm font-bold text-muted-foreground">{i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `${i + 1}`}</TableCell>
                      ...
                      <TableCell className="text-center"><Badge className={`text-[10px] border-0 ${r.sla >= 93 ? "bg-success/15 text-success" : r.sla >= 85 ? "bg-warning/15 text-warning" : "bg-destructive/15 text-destructive"}`}>{r.sla}%</Badge></TableCell>
                      ...
                      <TableCell className="text-center">
                        <div className="flex items-center gap-1 justify-center">
                          <div className="h-2 w-16 bg-muted rounded-full overflow-hidden"><div className={`h-full rounded-full ${r.efficiency >= 85 ? "bg-success" : r.efficiency >= 70 ? "bg-warning" : "bg-destructive"}`} style={{ width: `${r.efficiency}%` }} /></div>
        ...
        <TabsContent value="clients">
          <Card>
            <CardHeader className="py-3 px-4 border-b"><CardTitle className="text-sm font-semibold">Análisis de Clientes B2B — Febrero 2026</CardTitle></CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Cliente</TableHead><TableHead className="text-center">Pedidos/día</TableHead><TableHead className="text-right">Ticket prom</TableHead>
                    <TableHead className="text-right">Revenue mes</TableHead><TableHead className="text-center">Crecimiento</TableHead><TableHead className="text-right">LTV anual est.</TableHead>
                    <TableHead className="text-center">Riesgo churn</TableHead><TableHead>Pago</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {clientAnalytics.map((c) => (
                    <TableRow key={c.clientId}>
                      ...
                      <TableCell className="text-center"><Badge className={`text-[10px] border-0 ${c.churnRisk === "Bajo" ? "bg-success/15 text-success" : c.churnRisk === "Medio" ? "bg-warning/15 text-warning" : "bg-destructive/15 text-destructive"}`}>{c.churnRisk}</Badge></TableCell>
                      <TableCell>
                        {c.status === "paid" ? <Badge className="text-[10px] bg-success text-success-foreground">Al día</Badge>
                          : c.status === "partial" ? <Badge className="text-[10px] bg-warning text-warning-foreground">Parcial</Badge>
